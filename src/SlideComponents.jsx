...
// ════════════════════════════════════════════════════════════════════════════
// BUILD PANEL — Copy prompt → paste into Claude.ai (no API key required)
// ════════════════════════════════════════════════════════════════════════════
export function BuildPanel({ build, copiedTask, onCopy }) {
  if (!build) return <div style={{ color:'#94A3B8', fontSize:12, textAlign:'center', marginTop:30 }}>No build challenge for this slide.</div>

  // Build a ready-to-paste prompt per task by combining the scenario + that specific task
  const fullPrompts = build.tasks.map(t => `${build.scenario}\n\n${t}`)
  ...
  return (
    <div>
      <div style={{ background:'#0A1628', border:'1px solid #2E3347', borderRadius:10, padding:16, marginBottom:14 }}>
        ...
        <div style={{ fontSize:12.5, color:'#c8d0e0', lineHeight:1.6 }}>{build.scenario}</div>
      </div>

      <div style={{ background:'rgba(30,64,175,0.08)', border:'1px solid rgba(30,64,175,0.25)', borderRadius:8, padding:12, marginBottom:14, display:'flex', alignItems:'flex-start', gap:8 }}>
        <span style={{ fontSize:14 }}>💡</span>
        <div style={{ fontSize:11.5, color:'#BFDBFE', lineHeight:1.6 }}>
          Tap "Copy Prompt" on any task below, then paste it into a Claude.ai chat (open a new tab) to get the AI's response. Bring back anything interesting to share with the group.
        </div>
      </div>

      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {build.tasks.map((t, ti) => (
          <div key={ti} style={{ background:'#152347', border:'1px solid #2E3347', borderRadius:10, padding:14 }}>
            <div style={{ display:'flex', gap:8, marginBottom:10 }}>
              <span style={{ background:'#F59E0B', color:'#0D1B3D', borderRadius:'50%', width:20, height:20, display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:700, flexShrink:0 }}>{ti+1}</span>
              <div style={{ fontSize:12.5, color:'#e8eaf0', lineHeight:1.55 }}>{t}</div>
            </div>
            <button onClick={() => onCopy(fullPrompts[ti], ti)} style={{
              background: copiedTask === ti ? '#0D9488' : '#1E40AF', color:'#fff', border:'none',
              padding:'8px 16px', borderRadius:6, fontSize:11.5, fontWeight:600, cursor:'pointer',
              display:'flex', alignItems:'center', gap:6
            }}>
              {copiedTask === ti ? '✓ Copied to Clipboard' : '📋 Copy Prompt'}
            </button>
          </div>
        ))}
      </div>
      ...
      <a href="https://claude.ai" target="_blank" rel="noopener noreferrer" style={{
        display: 'block', textAlign: 'center', marginTop: 14, background: 'transparent',
        border: '1px solid #2E3347', color: '#94A3B8', padding: 10, borderRadius: 8,
