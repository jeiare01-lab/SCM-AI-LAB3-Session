...
// ════════════════════════════════════════════════════════════════════════════
// BUILD PANEL — AI assistant challenge
// ════════════════════════════════════════════════════════════════════════════
export function BuildPanel({ build, aiInput, setAiInput, aiResponse, aiLoading, onAsk }) {
  ...
  return (
    <div>
      <div style={{ background:'#0A1628', border:'1px solid #2E3347', borderRadius:10, padding:16, marginBottom:14 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
          {build.tasks.map((t,ti)=>(
            <div key={ti} style={{ display:'flex', gap:8, fontSize:11.5, color:'#94A3B8' }}>
              <span style={{ background:'#F59E0B', color:'#0D1B3D', borderRadius:'50%', width:16, height:16, display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, fontWeight:700, flexShrink:0, marginTop:1 }}>{ti+1}</span>
              {t}
            </div>
          ))}
        </div>
      ...
      <textarea
        ...
        style={{ width:'100%', minHeight:90, background:'#152347', border:'1px solid #2E3347', borderRadius:8, padding:12, color:'#fff', fontSize:13, outline:'none', resize:'vertical', boxSizing:'border-box', fontFamily:"'Inter',sans-serif" }}
      />
      <button onClick={onAsk} disabled={aiLoading || !aiInput.trim()} style={{
        marginTop:10, background: aiLoading ? '#475569' : '#1E40AF', color:'#fff', border:'none', padding:'10px 20px',
        borderRadius:7, fontSize:12, fontWeight:600, cursor: aiLoading?'not-allowed':'pointer'
      }}>
        {aiLoading ? 'Thinking...' : '✨ Ask PGB AI Assistant'}
      </button>
      {aiResponse && (
