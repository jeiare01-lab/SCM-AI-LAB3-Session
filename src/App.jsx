...
export default function App() {
  ...
  // ── PARTICIPANT: ask Claude AI assistant (Build tab) ──────────────────────
...
Be specific, practical, and reference actual PGB entities (AMICI, AAC, PSC, etc.) when relevant. Keep answers concise and actionable. Use Filipino business context where appropriate.`
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      setAiResponse('Error connecting to AI assistant. Please check your connection and try again.')
    } finally {
      setAiLoading(false)
    }
  }
            ...
            <div style={{ background: '#0f1520', borderRadius: '0 0 10px 10px', padding: 14 }}>
              ...
              {activeTab === 'build' && (
                <BuildPanel
                  ...
                  aiResponse={aiResponse} aiLoading={aiLoading}
                  onAsk={handleAskAI}
                />
              )}
              {activeTab === 'present' && (
