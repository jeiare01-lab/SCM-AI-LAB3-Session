...
export default function App() {
  ...
  // ── PARTICIPANT: ask AI via Vercel serverless proxy (no key in browser) ───
  const handleAskAI = async (promptText) => {
    ...
    try {
      ...
      setAiError('Could not reach the AI assistant. Check your connection and try again.')
    } finally {
      setAiLoading(false)
    }
  }
            ...
            <div style={{ background: '#0f1520', borderRadius: '0 0 10px 10px', padding: 14 }}>
              ...
              {activeTab === 'build' && (
                <BuildPanel
                  ...
                  aiResponse={aiResponse} aiLoading={aiLoading} aiError={aiError}
                  onAsk={handleAskAI}
                />
              )}
              {activeTab === 'present' && (
